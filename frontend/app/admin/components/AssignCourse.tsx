      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="grid gap-4 px-6 py-4">
          <div className="grid gap-1.5">
            <label htmlFor="course" className="text-sm font-medium text-slate-700">Select Course</label>
              ...
              <SelectContent>
                {courses.map((course) => (
                  <SelectItem key={course.course_id} value={course.course_id.toString()}>
                    {course.course_name} · {course.semester_name} ({course.credit_hours} hrs)
                  </SelectItem>
                ))}
          ...
          {selectedCourseData && (
            <div className="p-3 bg-blue-50 border border-blue-100 rounded-lg text-sm space-y-1">
              <p><span className="font-semibold text-blue-900">Course:</span> <span className="text-slate-700">{selectedCourseData.course_name}</span></p>
              <p><span className="font-semibold text-blue-900">Credit Hours:</span> <span className="text-slate-700">{selectedCourseData.credit_hours}</span></p>
              <p><span className="font-semibold text-blue-900">Enrollment:</span> <span className="text-slate-700">{selectedCourseData.registered_students}/{selectedCourseData.max_registered_students} students</span></p>
              <p><span className="font-semibold text-blue-900">Course ID:</span> <span className="text-slate-700">{selectedCourseData.course_id}</span></p>
            </div>
          )}
        </div>
